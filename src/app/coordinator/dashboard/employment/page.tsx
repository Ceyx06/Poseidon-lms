...
function getOpenFileUrl(file: EmploymentRecord): string {
  ...
  if (/\.(doc|docx|xls|xlsx|ppt|pptx)$/i.test(file.fileName))
...
function mapApiRecord(r: any): EmploymentRecord {
  return {
    ...
    publicId:
      typeof (r.public_id ?? r.publicId) === "string"
        ? String(r.public_id ?? r.publicId)
        : undefined,
      ...
      const res = await fetch("/api/send-email", {
        ...
        body: JSON.stringify({
          ...
          files: selectedFiles.map((f) => ({
            ...
            crewName: f.crewName, fileSize: f.fileSize || "",
          })),
        }),
      });
...
function FolderPanel({
  ...
  folder: { crewName: string; crewKey: string; items: EmploymentRecord[] };
  onClose: () => void;
  checkedIds: Set<string>;
  toggleCheck: (id: string) => void;
  toggleFolderCheck: (files: EmploymentRecord[]) => void;
  deletingIds: Record<string, boolean>;
  handleDelete: (r: EmploymentRecord) => void;
  ...
  const allChecked = folder.items.length > 0 && folder.items.every((f) => checkedIds.has(f.id));
  const someChecked = folder.items.some((f) => checkedIds.has(f.id));
      ...
      <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "min(800px, 96vw)", background: "#fff", boxShadow: "-10px 0 50px rgba(10,25,45,0.2)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ background: "linear-gradient(135deg,#0f2742,#1a3a5c)", padding: "18px 24px", display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ margin: 0, fontSize: 10, color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 700 }}>Employment Certs</p>
        ...
        <div style={{ padding: "12px 24px", borderBottom: "1px solid #e8eef5", background: "#f8fbff", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", flexShrink: 0 }}>
          ...
          <button type="button" onClick={() => toggleFolderCheck(folder.items)}
                ...
                return (
                  <div key={file.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 12, border: `1px solid ${isChecked ? "#bfdbfe" : "#e8eef5"}`, background: isChecked ? "#f0f7ff" : index % 2 === 0 ? "#fff" : "#fbfdff" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ margin: "2px 0 0", fontSize: 11, color: "#7a8fa5" }}>
                        {file.fileSize}
                        {file.createdAt ? ` · ${new Date(file.createdAt).toLocaleDateString("en-PH", { month: "short", day: "numeric", year: "numeric" })}` : ""}
                      </p>
                    </div>
...
export default function EmploymentPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!mounted) return;
        setRecords((data?.records ?? []).map(mapApiRecord));
      } catch (err) {
        if (!mounted) return;
        setRecords([]);
  ...
  async function uploadSingleFile(file: File, name: string): Promise<EmploymentRecord> {
    ...
    const metaRes = await fetch("/api/employment", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        crewName: name.trim(), vesselName: name.trim(),
        rank: "", principal: "", contractStart: null, contractEnd: null,
        certNumber: "", issuedDate: null, issuedBy: "", remarks: "",
        fileName: file.name, fileUrl: uploadData.url,
        fileSize: formatFileSize(file.size), publicId: uploadData.publicId,
      }),
    });
  ...
  async function handleUpload() {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      try {
        newRecords.push(await uploadSingleFile(selectedFiles[i], crewName));
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter(
      (r) => r.crewName.toLowerCase().includes(q) || r.fileName.toLowerCase().includes(q)
    );
  }, [records, search]);

  const folders = useMemo(() => {
    const map = new Map<string, { crewName: string; crewKey: string; items: EmploymentRecord[] }>();
    for (const rec of filteredRecords) {
      const key = toCrewKey(rec.crewName);
      const ex = map.get(key);
      if (ex) ex.items.push(rec);
      else map.set(key, { crewName: rec.crewName, crewKey: key, items: [rec] });
    }
    return Array.from(map.values()).sort((a, b) => a.crewName.localeCompare(b.crewName));
  }, [filteredRecords]);

  const openFolder = useMemo(
    () => folders.find((f) => f.crewKey === openFolderKey) ?? null,
  ...
  const FOLDER_COLORS = [
    "#b8841f", "#1a6bbf", "#1a7a4a", "#7c3aed",
    "#c0392b", "#0e7490", "#be185d", "#0369a1",
  ];
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "60vh", flexDirection: "column", gap: 10 }}>
        ...
        <p style={{ margin: 0, color: "#6a85a0", fontSize: 14 }}>Loading certificates...</p>
    ...
    <div style={{ fontFamily: "var(--font-dm)", background: "linear-gradient(180deg,#f6f9fc 0%,#fff 55%,#f8fafc 100%)", minHeight: "100vh", padding: 22 }}>
      ...
      <div style={{ maxWidth: 1120, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <p style={{ margin: 0, fontSize: 14, color: "#5a6f86" }}>Click any folder to view and manage its employment files.</p>
        </div>
        ...
        {loadError && (
          <div style={{ marginBottom: 16, padding: "12px 14px", borderRadius: 10, background: "#fff7ed", border: "1px solid #fed7aa", color: "#9a3412", fontSize: 13 }}>
            Records could not be loaded: {loadError}
          </div>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12, marginBottom: 18 }}>
          {[
            { label: "Total Folders", value: new Set(records.map((r) => toCrewKey(r.crewName))).size, accent: "#1a6bbf", bg: "#eef4ff" },
        ...
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid rgba(201,151,42,0.25)", padding: 20, marginBottom: 20, boxShadow: "0 8px 18px rgba(201,151,42,0.08)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(250px,1fr))", gap: 14, marginBottom: 12 }}>
            ...
            <div>
              <label style={{ display: "block", fontSize: 11, fontFamily: "var(--font-cinzel)", fontWeight: 700, color: "#5f6b7a", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>
                Certificate Files * <span style={{ fontSize: 10, color: "#1a6bbf", fontFamily: "var(--font-dm)", textTransform: "none", letterSpacing: 0 }}>(select multiple)</span>
              </label>
        ...
        {/* ── Search ── */}
        <div style={{ marginBottom: 16 }}>
          <input placeholder="Search folders by crew name..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: "1px solid #d0dce8", fontSize: 13, color: "#102a43", background: "#fff", outline: "none", boxSizing: "border-box", boxShadow: "0 2px 8px rgba(15,39,66,0.05)" }} />
        </div>
        ...
        {folders.length === 0 ? (
          <div style={{ textAlign: "center", padding: "64px 20px", color: "#7a8fa5", border: "1px dashed #d4e0ec", borderRadius: 14, background: "#f9fcff" }}>
            ...
            <p style={{ fontSize: 15, fontFamily: "var(--font-cinzel)", color: "#17324d", margin: "0 0 6px" }}>
              {search ? "No folders match your search." : "No employment certificates yet."}
            </p>
