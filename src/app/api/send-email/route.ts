...
function normalizeFileUrl(input: string): string {
  ...
  try {
    ...
    // If legacy proxy URL is stored (localhost/vercel -> /api/crew-documents/file?url=...)
    if (parsed.pathname === "/api/crew-documents/file") {
...
function getEmailOpenUrl(fileName: string, fileUrl: string): string {
  ...
  if (lower.endsWith(".ppt") || lower.endsWith(".pptx") || lower.endsWith(".xls") || lower.endsWith(".xlsx")) {
...
function buildEmailHtml(files: FileItem[], message?: string): string {
  const fileRows = files
    .map(
      (f) => `
      <tr>
        <td style="padding:12px;border-bottom:1px solid #e8eef5;">
          <strong style="color:#102a43;font-size:13px;display:block;line-height:1.35;word-break:break-word;">
            ${f.fileName}
          </strong>
          <span style="color:#6a85a0;font-size:11px;display:block;margin-top:3px;word-break:break-word;">
            ${f.crewName} - ${f.fileSize}
          </span>
          <a href="${getEmailOpenUrl(f.fileName, f.fileUrl)}"
             style="display:block;width:100%;box-sizing:border-box;text-align:center;margin-top:10px;
                    background:#1a6bbf;color:#fff;padding:9px 12px;border-radius:8px;
  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f6f9fc;padding:24px;border-radius:12px;">
      ...
      <div style="background:#fff;padding:20px 24px;border:1px solid #dbe5f0;border-top:none;">
        ${message ? `<p style="color:#102a43;font-size:14px;margin:0 0 16px;word-break:break-word;">${message}</p>` : ""}

        <p style="color:#5a6f86;font-size:13px;margin:0 0 12px;">
          ${files.length} document${files.length > 1 ? "s have" : " has"} been shared with you:
        </p>

        <table style="width:100%;border-collapse:collapse;border:1px solid #e3ebf4;border-radius:8px;overflow:hidden;table-layout:fixed;">
          <thead>
